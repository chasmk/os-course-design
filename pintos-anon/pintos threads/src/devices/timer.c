...
这两句之间的语句将被保证不会被中断，以维持原子性操作
  实现了：禁用了中断->保存现场->恢复原来中断状态的变化*/
int64_t
timer_ticks (void) 
{
    /*
   intr_level 代表能否被中断
   intr_disable ()获取了当前的中断状态， 然后将当前中断状态改为不能被中断， 然后返回执行之前的中断状态。
    old_level保存禁止被中断前的中断状态
 */
  enum intr_level old_level = intr_disable ();
  int64_t t = ticks;//代表着操作系统执行单位时间的前进计量
...
timer_sleep (int64_t ticks) 
{
    ...
    intr_set_level(old_level);
//imer_sleep就是在ticks时间内， 如果线程处于running状态就不断把他扔到就绪队列不让他执行。
