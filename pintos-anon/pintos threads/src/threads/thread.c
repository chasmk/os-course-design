               ...
               thread_func *function, void *aux) 
{
  ...
  /*for prog 1 threads alarm clock  初始化sleep_ticks*/
  t->sleep_ticks = 0;

  /*for prog 2 userprog */
  //t->threading_started = false;
...
thread_current (void) 
{
  struct thread *t = running_thread ();//running_thread的结果是返回当前线程起始指针。
  ...
  ASSERT (is_thread (t));//断言t指针是一个线程
  ASSERT (t->status == THREAD_RUNNING);//断言这个线程处于THREAD_RUNNING状态
...
thread_yield (void) 
{
  ...
  old_level = intr_disable ();
  if (cur != idle_thread) //如果当前线程不是空闲
      list_insert_ordered(&ready_list, &cur->elem, (list_less_func*)&thread_cmp_priority, NULL);
    //list_push_back (&ready_list, &cur->elem);//把当前线程的元素扔到就绪队列
